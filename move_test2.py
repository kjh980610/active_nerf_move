...
def go_joint_paths(joint_path):
    go_to_joint_state(joint_path[-1])
...
def go_to_joint_state(joint_pose):
    ...
    if joint_pose is not None :
        for i, joint_pose in enumerate(joint_pose):
            pose['panda_joint'+str(i+1)] = joint_pose
